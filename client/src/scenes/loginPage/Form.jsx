...
const registerSchema = yup.object().shape({
  ...
  username: yup.string().required("required").min(2).max(50),
  picture: yup
    .mixed()
    .required("required")
    .test(
...
const Form = () => {
  ...
  const register = async (values, onSubmitProps) => {
    try {
      ...
      // Log the REACT_APP_API_URL value
      ...
      onSubmitProps.setErrors({ 
        ...
        email: "Registration failed" // Show error under email field
      });
    }
  };
  ...
  const login = async (values, onSubmitProps) => {
    try {
      ...
      if (!loggedInResponse.ok) {
        throw new Error(loggedIn.message || 'Login failed');
      }
      ...
      onSubmitProps.setErrors({ usernameOrEmail: "Invalid credentials" });
    }
  };
